...
// 🏆 Fetch all teams and registered players
async function loadData() {
  const { data: players } = await supabase.from("registrations").select("*");
...
function populateTeams(teams, players) {
  ...
  const taken = players.map(p => p.team_name);
  teams.forEach(t => {
...
form.addEventListener("submit", async e => {
  ...
  const { error } = await supabase.from("registrations").insert([
    { username, whatsapp, team_name: team }
  ]);
